...
Nach dem das Gerät am Seil angebracht wurde, kann mit dem Restart-Knopf am \textit{ArduinoDue} gestartet werden. Es wird nun eine Vorprogrammierter Ablauf gefahren. Dieser beinhaltet den Antriebsschrittmotor zum Vorwärtsfahren bis der Haken in die Last eingehängt hat und den Hubschrittmotor, welcher die Last um eine bestimmte Schrittanzahl hebt. Dieser Ablauf läuft über das \textit{ArduinoDue}. Gleichzeitig werden die Daten der Ultraschallsensoren über das \textit{RasPi} ausgewertet und als Distanzen in Millimetern vom \textit{ArduinoDue} abgefragt. Bei der z-Koordinate wird zusätzlich noch die vom Hubschrittmotor gehobene Höhe abgezogen und beide Koordinaten werden auf dem LCD-Display angezeigt.\\
Das Gerät wird dann mit der aufgenommenen Last Vorwärtsfahren, bis die Kamera die Zielplattform sieht und das \textit{RasPi} diese erkennen kann. Das \textit{ArduinoDue} fragt regelmäßig ab, ob das \textit{Raspi} die Zielplattform bereits erkannt hat. Falls die Antwort ein positiver Wert ist, wird diese übermittelte Distanz abgefahren und der Hubmotor startet das Absetzen der Last. Wie viel der Hubmotor die Last absenken muss, wird aus den Daten des vertikal-messenden Ultraschallsensor und den Schritten des Hubmotors berechnet. Wenn die Last auf der Zielplattform aufkommt, klinkt sich der Haken automatisch aus der Last aus. Der Haken wird danach wieder 25 Zentimeter hochgehoben und Der Antriebsmotor fährt solange vorwärts, bis der Endschalter betätigt wird. Damit ist die Aufgabe beendet.
...
\subsubsection{Kraftübertragung und Führung}
...
\subsubsection{Kamerahalterung} %todo Abmasse halterung anpassen
...
\begin{longtable}{|p{.05\textwidth}|p{.05\textwidth}|p{0.2\textwidth}|p{0.25\textwidth}|p{0.25\textwidth}|}
	...
	\hline
	2.02 & 3.7 3.8 & Hub- und Fahrmotor ansteuern & Dezember 2017& Systemkomponenten können zugekauft werden \\
	\hline
	...
	\hline
	2.06 & 3.7 & Laufkatze + Antrieb mit Schrittmotor & Test des Schrittmotors für den Antrieb. Dabei soll die Funktion des Schrittmotors des Antriebes und das Verhalten des Antriebes / Laufkatze soll getestet werden & Die Laufkatze ist gefahren \\
	\hline
