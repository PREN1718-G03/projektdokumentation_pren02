...
Da das Arduino mit einer Spannung von bis zu 5V auf seinen UART Pins arbeitet und das Raspberry Pi mit nur 3.3V muss ein Logikpegelübersetzer eingesetzt werden. Wir verwenden dazu ein TXB0108 von Adafruit.
...
Die Laufkatze hat total zwei Schrittmotoren verbaut. Einen für den Antrieb auf dem Seil und einen für den Hub der Last. 
...
\begin{itemize}
		\item Der Haken ist neu aus Kunststoff aus dem 3D-Drucker gefertigt. Aufgrund einer leicht veränderten Geometrie ist 3D-Druck sinnvoller.
...
Bevor die Komponenten in das Gesamtsystem integriert wurden, wurden sie einzeln getestet. Danach wurden sie ins Gesamtsystem integriert und erneut getestet. Für sämtliche Tests wurde immer zuerst ein Ziel definiert und dann die Resultate mithilfe eines Protokolls festgehalten.

\subsection{Testszenarien}

Aufgrund der grossen Anzahl Testszenarien und Testfälle werden diese übersichtshalber mit den
Testresultaten im Anhang aufgeführt.
...
%- Raupe zieht es ab Rädern:
%Bei zu hohem Grip der Raupe wird bei einer Drehung (z.B. im Wendebereich) die Raupe
%von den Rädern gezogen.
%- Kugeldämpfung eher Federung:
%In den Tests wurde festgestellt, dass die Kugeldämpfung eher als Federung agiert. Als
%Gegenmassnahme wurden dämpfende Materialien unter dem Federarm platziert und die
%Geschwindigkeit auf der Wippe verringert.
%- Servoarm verändert Kalibrationsposition:
%Der Servo fährt mit demselben PWM-Signal unterschiedliche Positionen an.
%- Ziffernerkennung in seltenen Fällen: In den Tests wurde festgestellt, dass z.B. direkt
%beim Start die Ziffer aufgrund der laufenden Kalibration nicht erkannt wird. Teilweise
%wurden die Ziffern unterwegs nicht erkannt, was auf nicht passende Rotwerte bei der
%Balkenerkennung zurückzuführen ist.
%Mehrere Testläufe haben gezeigt, dass das Gerät in 7/10 Durchläufen alle Anforderungen erfüllt.
%Es gingen bei der Überquerung des Parcours keine Kugeln verloren, die Ziffer wurde korrekt
%erkannt und angezeigt sowie im Zielfeld korrekt gewählt.
