...
Das Gerät muss sich autonom entlang eines ansteigenden Drahtseils fortbewegen können. Eine Last, welche sich an einem vorbestimmten Platz befindet soll vom Gerät angehoben werden. Ohne Berührung der nachfolgenden Hindernisse muss die Last exakt auf der Zielplatte, welche sich im Absetzbereich befindet, abgesetzt werden (siehe Abb.\ref{fig:Funktionsskizze}).
...
Nachdem das Gerät am Seil angebracht wird (\ref{sec:Bedienungsanleitung}), kann mit dem Reset-Knopf des \textit{ArduinoDue} (\ref{sssec:ArduinoDue}) gestartet werden. Es wird nun ein vorprogrammierter Ablauf gefahren. Dieser beinhaltet den Antriebsschrittmotor zum Vorwärtsfahren (\ref{ssec:Antrieb}) bis der Haken in die Last eingehängt hat (\ref{ssec:Lastgreifenundabsetzen}) und den Hubschrittmotor, welcher die Last um eine bestimmte Schrittanzahl hebt (\ref{ssec:Lastanhebenundsenken}). Dieser Ablauf läuft über das \textit{ArduinoDue}. Gleichzeitig werden die Daten der Ultraschallsensoren (\ref{ssec:Ultraschallsensor}) über das \textit{RasPi} (\ref{sssec:RasPi}) ausgewertet und als Distanzen in Millimetern vom \textit{ArduinoDue} abgefragt. Bei der z-Koordinate wird zusätzlich noch die vom Hubschrittmotor gehobene Höhe abgezogen und beide Koordinaten werden auf dem LCD-Display (\ref{ssec:Positionsanzeige}) angezeigt.\\
Das Gerät wird dann mit der aufgenommenen Last vorwärts fahren, bis die Kamera (\ref{ssec:Zielerkennung}) die Zielplatte sieht und das \textit{RasPi} diese erkennen kann (\ref{ssec:Zielerkennung}). Das \textit{ArduinoDue} fragt regelmässig ab, ob das \textit{RasPi} die Zielplatte bereits erkannt hat. Falls die Antwort ein positiver Wert ist, wird diese übermittelte Distanz abgefahren und der Hubmotor startet das Absetzen der Last. Wie viel der Hubmotor die Last absenken muss, wird aus den Daten des vertikal messenden Ultraschallsensors und den geleisteten Schritten des Hubmotors berechnet. Wenn die Last auf der Zielplatte aufkommt, klinkt sich der Haken automatisch aus der Last aus. Der Haken wird danach wieder 25 Zentimeter hochgehoben und der Antriebsmotor fährt solange vorwärts, bis der Endschalter(\ref{ssec:Endschalter}) betätigt wird. Damit ist die Aufgabe beendet.
...
\begin{itemize}
	\item Das angetriebene Laufrad ist neu aus Gummi und nicht nur mit Gummi beschichtet. Die Reibung auf dem Seil wird damit vergrössert und Rutschen auf dem Seil wird vermindert.
	...
	\item Die Grundplatte wird aus bearbeitungstechnischen Gründen nicht mehr aus Aluminium gefertigt sondern aus einer gelaserten, 6mm dicken Holzplatte.
	\item Die Positionsanzeige in x-Richtung wird nur noch über einen Ultraschallsensor bestimmt. Der Einbezug des Schrittmotor wird aufgrund der ausreichenden Genauigkeit des Ultraschallsensors nicht mehr benötigt.
	\item Die geplante Durchhangstabelle wird nicht miteinbezogen, da ebenfalls ein Ultra\-schall\-sensor verwendet wird. Der Schrittmotor wird aber noch immer ein\-bezogen, da der Ultraschall\-sensor an der Grundplatte angebracht ist aber die Position der Last angezeigt werden muss.
\end{itemize}
...
Indem die Aufhängung gelenkig gelagert ist, kann die Winkeländerung am Seil kompensiert werden. Dadurch hängt die Laufkatze waagrecht, wenn der Schwerpunkt mittig unter dem Antriebsrad liegt. Durch das Gelenk kann die Laufkatze bei abrupten beschläunigungs und Bremsvorgängen ins pendeln geraten (ähnlich wie bei einer Gondelbahn). Das Gelenk wird mit einer Messingbuchse, einer Schraube und mit einer Sicherungsmutter realisiert. Ist das Gelenk, der Schwerpunkt der Laufkatze und der Haken nicht in einer Linie, so kommt es beim Anheben und Absetzen der Last zu einem unterschiedlichen Winkel der Laufkatze zum Boden. An der Aufhängung wird die Grundplatte angebracht.


\begin{figure}[h!]
	\centering
	\includegraphics[keepaspectratio,width=0.5\textwidth]{Antriebseinheit}
	\caption{Modulübersicht Aufhängung	}
	\label{fig:SystemArchitektur}
\end{figure}

\subsection{Antrieb}
\label{ssec:Antrieb}
Das mittlere Rad aus Gummi ist das Antriebsrad. Es wird direkt von einem Schrittmotor angetrieben. Das Antriebsrad wird mithilfe einer Hohlwelle direkt auf der Motorwelle befestigt. Die Ausfallwahrscheinlichkeit ist tief weil keine Übersetzungskomponenten notwendig sind. Umso grösser das Antriebsrad ist, desto höher ist der Schwerpunkt des gesamten Laufkatze. Der Lastausgleich wird daher mit einem kleineren Durchmesser des Antriebsrades begünstigt.
\\
...
Die Laufkatze hat total zwei Schrittmotoren verbaut. einen für den Antrieb auf dem Seil und einen für den Hub der Last. Der Antrieb auf dem Seil ist ein Motor des Typs JK42HM48-1684. Dieser dreht sich pro Schritt um \SI{0.9}{\degree}. Das Haltemoment beträgt 4.4kgcm. Der Antrieb des Hubes wird mit einem Motor des Typs 35HS3408A4 ausgeführt. Dieser dreht sich pro Schritt um \SI{1.8}{\degree}. Das Haltemoment beträgt 1.8kgcm.
\parencite{CNCSHOP2014}, \parencite{OWO}
\\
In Abbildung \ref{fig:SystemArchitektur} ist die Antriebseinheit dargestellt. Diese ist für den Antrieb auf dem Seil und für die Befestigung des ganzen Gerätes auf dem Seil konzipiert. Die Antriebseinheit wird über das Gelenk mit dem Rest der Laufkatze verbunden.

\subsection{Grundplatte}
\label{ssec:GrundplatteBeschrieb}

Die Grundplatte wurde aus einer MDF-Platte gefertigt. Diese Platte hat einen Ausschnitt in der Mitte, durch welchen das Seil der Seilwinde durchgeführt ist. Neben dem Ausschnitt wird der \textit{Verbindungswinkel} (\ref{sssec:VerbindungswinkelBeschrieb}) angeschraubt. Zwei zusätzliche Löcher links und rechts neben dem Ausschnitt dienen zur Befestigung der beiden Führungen des Hakens, welcher mit zwei Kohlefaserstäben in Z-Richtung gleiten kann. Die Elektronikkomponenten werden mithilfe von Klettstreifen auf die Grundplatte befestigt. Die Halterung für den LCD-Display wird mit einer Steckverbindung auf die Grundplatte gesteckt. Zusätzlich sind zwei kleine Winkel aus Aluminium angebracht, um Schalter für die Mikrocomputer zu befestigen. Einer davon ist oben und der andere unten an der Grundplatte befestigt. Auf der Unterseite der Grundplatte wir die Halterung für den Ultraschallsensor mit vier Schrauben befestigt. Die Kabelführung ist mithilfe von Klebesockel auf der Grundplatte gelöst.
...
Der Verbindungswinkel ist die Verbindung der Grundplatte und der Antriebseinheit. Dabei wird der Verbindungswinkel mit zwei M5 Schrauben an die Grundplatte geschraubt. Der Verbindungswinkel besitzt ein Gleitlager aus Messing.

\subsubsection{Kamerahalterung}
\label{sssec:Kamerahalterung}
Die Kamera ist 20 Zentimeter vor der Seilwinde für den Hub angebracht. Dafür wurde ein 11 Zentimeter langes Winkelprofil aus Aluminium an der Grundplatte angeschraubt. An diesem ist ein Aluminiumwinkel befestigt, der die Kamera hält. Dieser ist schwenkbar, damit die Kamera optimal ausgerichtet werden kann. Dabei ist essentiell wichtig, dass die Grundplatte immer Parallel zum Boden liegt, da sonst die Kamera falsche Daten liefern würde und somit die Genauigkeit beim Absetzen stark beeinträchtigen würde. Die \textit{RasPi}-Kamera wird mit 4 Schrauben an dem Winkel befestigt. Zusätzlich ist auf dem Winkelprofil der Ultraschallsensor in x-Richtung angebracht und auf dem Aluminiumwinkel der Endschalter.
...
Das Greifen der Last erfolgt über einen gelenkig gelagerten Haken.  Eine nicht-Drehbare Querstange aus Aluminium dient als Halterung für den Haken. Ebenfalls an dieser Querstange ist ein Schwenkbegrenzer mit einer Druckfeder befestigt. Die Feder hält den Haken in einer Schieflage von etwa 45\SI{}{\degree}. Weil die Startposition der Last vorgegeben ist, wird das Aufheben der Last statisch programmiert.
...
Beim Absetzen der Last auf der Zielplatte drückt die Feder den Haken automatisch wieder in die Ursprungsposition und klinkt sich damit aus dem Haken der Last aus.
...
Da ein loses Seil stark in Schwingung geraten kann, wird zusätzlich eine Führung mit zwei Stäben aus Kohlenstofffasern verwendet. Auf der Befestigungsplatte sind dafür Führungsrohre angebracht. Aufgrund dieser Führungsstäbe können Schwingungen und Verdrehungen vermieden werden.
...
Als Kern der elektronischen Steuerung wird das \textit{ArduinoDue} Board verwendet, welches mit dem \textit{Atmel ATSAM3X8E} Mikroprozessor bestückt ist. Programmiert wird in C.
...
Weil die Steuerung vom RasPi die Richtung und Distanz zur Zielplatte misst und übermittelt, kann das System als Regelungssystem betrachtet werden. Solange das Ziel nicht erkannt wird, soll das Gerät mit maximaler Geschwindigkeit fahren. Sobald die Zielplatte erkennt wird, fährt die Laufkatze ans Ziel und hält dort an, damit die Last abgesetzt werden kann. Mit diesem System wird ein effektives Anfahren des Ziels erreicht. Zum Starten des Ablaufs wird ein Schalter und zum Stoppen wird ein Taster verwendet.
...
Zwischen dem RasPi und dem \textit{ArduinoDue} soll der Informationsaustausch über eine serielle Schnittstelle erfolgen.

Für die Kommunikation mittels asynchroner, seriellen UART-Schnittstelle können, auf dem \textit{ArduinoDue} wie auf dem RasPi, die explizit dafür ausgelegten PINs verwendet werden.

Wie in Abb. \ref{fig:RaspberryPins} dargestellt sind dies Pin 18 (Tx) und Pin 19 (Rx). Zusätzlich müssen die beiden Ground (GND) Potentiale vom RasPi und dem \textit{ArduinoDue} verbunden sein.
\begin{figure}[h!]
	\centering
...
Um den aktuellen Stand (das Abbild, Systemimage) des RaspberryPi zu speichern und gegebenenfalls wiederherstellen zu können, wird die SD Karte in ein Studentennotebook gelegt und mithilfe des Win32 Disk Imager kopiert.
...
\begin{figure}[h!]
	\centering
	\includegraphics[keepaspectratio,height=0.6\textheight]{Ablaufdiagramm_ModusOperandi}
...
\begin{figure}[h!]
	\centering
	\includegraphics[keepaspectratio,width=0.75\textwidth]{SystemArchitektur}
	\caption{Modulübersicht der PiSensor Architektur}
	\label{fig:PiSensorSystemArchitektur}
\end{figure}

Die Logik der Erkennung wurde in zwei Teile separiert, bildlich dargestellt in der Abbildung \ref{fig:KameraSensor}. Zum einen das Erfassen der Daten des Sensors und der Erkennung der Zielplatte, zum anderen das Berechnen der Distanz zwischen Zielplatte und der Kamera. Die Schnittstelle ausserhalb des Moduls bildet dabei der 'Sensor Controller', dessen Realisierung über eine Factory-Klasse bereitgestellt wird. Die eigentliche Logik findet in Subklassen der 'TargetRecognition' und 'DistanceCalculation' statt.

Im Moment wird die Erkennung der Zielplatte (wie in Kapitel \ref{sssec:Bilderkennung} dargestellt) über einen Konturen\-erkennungs\-algorithmus der Bibliothek 'OpenCV' realisiert. Ist die Zielplatte erkannt, so wird über den Controller das Signal zum Berechnen der Distanz gegeben. Die Methodik zur Distanzberechnung wird in Kapitel \ref{ssec:DistModelCreation} genauer erläutert.

Ist die Entfernung zur Zielplatte ermittelt, so soll der PiController genaue Fahrtanweisungen zur Motor- und Sensorsteuerung übergeben. Dies wird über das 'communicator'-Modul realisiert.
 
\begin{figure}[h!]
	\centering
	\includegraphics[keepaspectratio,width=0.75\textwidth]{pi-sensor_cs}
...
Für die Erkennung der Zielplatte wird die \textit{RasPi}-Kamera v2.1 verwendet. Angebracht ist die Kamera zuvorderst am Gerät mit vier Schrauben an einem Winkel. Damit die Zielplatte genügend früh erkannt werden kann und die Last nicht das Bild stört, ist die Kamera mit einem Winkel von \SI{20}{\degree} zur Vertikalen nach vorne gerichtet. Das Bild wird auf dem \textit{RasPi} ausgewertet und erkennt damit die Zielplatte.

\subsection{Positionsanzeige}
\label{ssec:Positionsanzeige}

Auf dem LCD-Display werden die Koordinaten in x-, sowie z-Richtung dargestellt. Zwei Ultraschallsensoren, je einer in x-, und z-Richtung, messen die jeweiligen Distanzen. Bei der z-Koordinate muss jedoch noch ein Betrag abgezogen werden, da der Ultraschallsensor an der Grundplatte befestigt ist und die Koordinaten der transportierten Last dargestellt werden sollen. Wie viel abgezogen werden muss, ist abhängig von der Distanz, welche vom Hubmotor angehoben wird. Damit bei der Fahrt über die Hindernisse nicht ein falscher Wert angezeigt wird, werden plötzliche grosse Änderungen nicht berücksichtigt und der vorherige Wert wird angezeigt. Der LCD-Display ist in Fahrtrichtung rechts mit Schrauben an einer Halterung aus dem 3D-Drucker befestigt. Die Halterung wiederum ist auf die Grundplatte gesteckt. Die Position wird einmal pro Sekunde aktualisiert. Die Anzeige ist in Zentimeter und mit einer Genauigkeit von einer Nachkommastelle. Das LCD-Modul wird über das \textit{ArduinoDue} angesteuert.

\newpage

\subsection{Ultraschallsensor}
\label{ssec:Ultraschallsensor}
Für die Distanzmessung werden in x-Richtung und y-Richtung je ein Ultraschallsensor verwendet. Die Ultraschallsensoren des Typs HC-SR04 weisen eine Messdistanz zwischen zwei Zentimeter und bis zu drei Meter auf. Die Genauigkeit wird mit drei Millimetern Abweichung angegeben. Dies ist ausreichend für die geplante Anwendung. Ausgelesen und verarbeitet werden die Informationen auf dem \textit{RasPi}.\\ \\ \parencite{UltraschallsensorHC-SR04}
...
\begin{itemize}
	\item Es muss sichergestellt sein, dass alle Kabel eingesteckt sind und der Hauptschalter sowie der Schalter für das \textit{ArduinoDue} ausgeschaltet sind.
	...
	\item Das Gerät muss so positioniert sein, damit der Masten gerade berührt wird.
	\item Die vorgegebene Höhe von 4 Zentimeter vom untersten Punkt des Hakens bis zum Boden muss eingestellt werden. Wenn sich der Haken auf der korrekten Höhe befindet, muss der Hauptschalter eingeschaltet werden, damit die Schrittmotoren ihre Position halten.
	\item Der Kippschalter am \textit{ArduinoDue} und dessen Resetknopf müssen betätigt werden um das Gerät zu starten.
\end{itemize}

\section{Schnittstellen und Abhängigkeiten}
\label{sec:SchnittAbhang}
% TODO Überprüfen
Die Schnittstellen der verschiedenen Komponenten sind zentral für die Funktionstüchtigkeit der Laufkatze. Zum einen ist dies die Kommunikation zwischen dem \textit{RasPi} und dem \textit{ArduinoDue}, zum anderen müssen die physikalischen Komponenten und die Steuerung miteinander verbunden sein. Die Schnittstelle zwischen den Mikrocomputern wird mit einer UART Steckverbindung realisiert. Es ist essenziell, dass das Protokoll auf den Mikrocomputern definiert ist, damit die Informationen richtig übermittelt werden. Diese Schnittstelle bietet sich an, weil sie auf beiden Mikrocomputern zur verfügung steht und sich zur Datenübertragung über kurze Distanzen eignet.

Viele Schnittstellen sind von der Positionsbestimmung abhängig. Um die Position zu bestimmen, werden die Schrittmotoren und zwei Ultraschallsensoren eingesetzt, welche die Daten liefern. Von der Positionsbestimmung abhängig ist das Greifen der Last, das Heben und Senken derselben und das Absetzen. Weil ein Haken eingesetzt wird, muss diese Positionsbestimmung genau sein, damit die Last abgesetzt werden kann. Die Genauigkeit in z-Richtung muss dabei in einer Toleranz von 0...+20mm liegen. In x-Richtung darf die Genauigkeit ebenfalls etwa 0...+20mm betragen, da sonst der Haken die Last nicht absetzt.

Damit die Zielerkennung mit OpenCV funktionieren kann, muss die Plattform möglichst ruhig und waagerecht sein. Dies ist abhängig von der Aufhängung und Antrieb. Deshalb muss auf die Anordnung der Komponenten geachtet werden, damit beim Lasttransport die Grundplatte waagrecht bleit. Da sich die Komponenten auf die Balance der Plattform auswirken, muss deren Position im voraus evaluiert werden.

Das Startsignal erfolgt über den Resetknopf am \textit{ArduinoDue}. Diese Variante ist weniger störungsanfällig als eine Signalerkennung via Bluetooth oder mittels einem akustischen Signal.
...
\begin{longtable}{|p{.05\textwidth}|p{.05\textwidth}|p{0.2\textwidth}|p{0.25\textwidth}|p{0.25\textwidth}|}
	...
	\hline
	2.03 & 4.6 4.7 4.8 & Prototyp Kommunikation & Ein funktioneller Prototyp der in der Lage ist seriell zu  kommunizieren & Es wurden zwei Skripte erstellt um die generelle Fähigkeit der Kommunikation zu erarbeiten \\
	\hline
	2.04 & 3.9 & Zielplatte Fläche und Distanz & Ziel des Tests soll sein, die Distanz eines Zentimeters (in Pixel) unter Berücksichtigung der Höhe zu ermitteln & Es konnte eine Liste mit Pixelwerten von 1 cm aus unterschiedlichen Höhen erstellt werden \\
	\hline
	...
	\hline
	2.08 & 4.6 4.7 & Ultraschall DUE LCD & \textit{ArduinoDue} Code zur Ansteuerung des Sensors in x-Achse(Länge) und z-Achse(Höhe) und des LCD-Displays mit Anzeige von beiden Längen & Der Sensor konnte auch auf dem \textit{ArduinoDue} betrieben werden, jedoch blockiert er sich teilweise selber. Die Genauigkeit der Messung konnte mithilfe eines Röhrchens das über den Sender gestülpt wurde verbessert werden.\\
	\hline
	...
	\hline
	2.11 & Alle Anforderungen & Alle Funktionen & Durchlauf aller Funktionen & Weil als Stromversorgung für das RasPi eine Powerbank verwendet wurde, wurde dieses nicht richtig mit Strom versorgt. Beim Nächsten Versuch muss die Stromquelle also auf die Akkus umgestellt werden.\\
	\hline
	2.12 & Alle Anforderungen & Alle Funktionen & Durchlauf aller Funktionen & Alle Teilanforderungen können erfüllt werden. Jedoch gibt es noch eine unsaubere Ausführung der Steuerung. Die Laufkatze fährt zuerst über die Zielplattform heraus, fährt wieder zurück und setzt die Last ab. Die Last ist auf der Zielplattform. Und der Endpfosten wird auch wie erwartet berührt.\\
	\hline

\end{longtable}
\footnotetext{Getestete Anforderungen aus Anforderungsliste (Siehe \ref{app:sec:ProjektAnf})}

Die Detaillierten Versuchsprotokolle sind im Anhang zu finden.

\section{Freigabetest}

Als Freigabetest wurde ein kompletter Testdurchlauf durchgeführt.\\

\subsection{Ablauf Test}
\begin{enumerate}
	\item Laufkatze auf Seil aufsetzen (Zeitfenster 2 Minuten)
	\item I/O-Schalter betätigen
	\item Vorwärtsfahren bis Last gegriffen
	\item Last heben
	\item Vorwärtsfahren bis Zielplattform erkannt
	\item Eingehende Werte umrechnen und entsprechenden Weg fahren
	\item Last auf Zielplattform absetzen
	\item Haken anheben
	\item Fahren bis Zielmasten erreicht
\end{enumerate}

\subsection{Resultate}

\begin{enumerate}
	\item Laufkatze kann in einer Minute auf das Seil gesetzt werden.
	\item Mit dem Betätigen des I/O-Schalters wird die Laufkatze gestartet
	\item Laufkatze greift Last
	\item Last wird vor dem 1. Hindernis gehoben
	\item Last wird über Hindernisse transportiert
	\item Zielplatte wird erkannt, Laufkatze fährt ca einen halben Meter übers Ziel und von da zurück über die Zielplatte.
	\item Last wird auf Zielplattform abgesenkt
	\item Haken wird Angehoben
	\item Gerät berührt Zielmast und Motoren wird gestoppt
	\item Während der gesamten Fahrt werden die Koordinaten angezeigt	\item Platform wird nicht berührt
\end{enumerate}

\begin{itemize}	
	\item Inbetriebnahmezeit ~eine Minute
	\item Fahrzeit 45 Sekunden
	\item Last auf Zweit-innersten Kreis abgesetzt
	\item getestete Fahrgeschwindigkeit 0.14 m/s
\end{itemize}

\subsection{Fazit}
Es konnten sämtliche für den Wettbewerb erforderlichen Tasks erfüllt werden. Jedoch ist die Fahrgeschwindigkeit im Moment noch zu tief um mit den schnelleren Teams mithalten zu können. Die Genauigkeit der Anzeige konnte auch nicht genau kontrolliert werden. Die Werte erschienen aber plausibel.
...
$\omega=v/r=6.67s^{-1}$	\\
Moment\\
$\underline{\underline{M}}=(m_L+m_V)*g*r=\underline{\underline{0.0368Nm}}$\\
Leistung	\\
...
Da wir unsere Kamera mit einem Anstellwinkel von \SI{70}{\degree} auf die Ebene schauen lassen, verzerren wir das Bild (und vor allem die Distanzen) perspektivisch (skizziert in Bild \ref{fig:Perspektive}). Um diese korrekt berechnen zu können wurde aus verschiedenen Höhen, die Länge in Zentimeter und in Pixel gemessen und durch die gesammelten Daten ein Modell gelegt. Dies wurde im Programm R Studio bewerkstelligt. Leider konnten die Daten nicht durch ein lineares Modell approximiert werden, und es musste ein Quadratisches verwendet werden, dies ist in Abbildung \ref{fig:Modelle} zu sehen.
...
Aufgrund Umstrukturierungen im Team, mussten wir in der vierten Woche des Moduls PREN2 unsere Planung drastisch anpassen. Daher orientierten wir uns an der Sprintplanung der SCRUM-Softwareentwicklung und organisierten uns in zweiwöchigen Sprints. Daher sind unsere Meilensteine relativ eng zueinander gesetzt.

Die detaillierte Planung finden Sie im Anhang in Kapitel \ref{ch:ProjektOrga}, darunter unter anderem die detaillierte Wochenplanung, Risikoplanung, Projektteamorganisation, und Tools welche zum Einsatz kamen.
...
Die Zusammenarbeit im Team war trotz einigen grösseren Schwierigkeiten bis zum Schluss sehr gut. Das Team war immer in der Lage sich gegenseitig zu unterstützen und Probleme zu beheben. Dies war unbedingt nötig, da Eve Meier (Informatik) nach dem Pren1 das Team verlassen hat und sich David Craven (Elektrotechnik) nach kurzer Zeit im Pren2 abgemeldet hat. Dadurch wurde das Team sehr unausgeglichen mit vier Maschinentechnikstudenten und nur je einem Informatik-, und Elektrotechnikstudenten. Die Studenten der Maschinentechnik versuchten zu helfen, wo es möglich war. Zum einen durch mithilfe beim Programmieren oder durch Anpassungen am Gerät um den Programmieraufwand zu reduzieren.\\
Nachfolgend werden nach Fachbereich aufgetrennt, die jeweiligen Erkenntnisse aus dem PREN-Modul beschrieben.

\section{Maschinentechnik}

\begin{itemize}
	\item Aufbau
	Es wurde viel untereinander über den Aufbau und Funktionsart gesprochen. Gute Ideen und Verbesserungen wurden so gefunden, da die Vorschläge angehört und diskutiert wurden.
	\item Kabelführung\\
...
\begin{itemize}
	...
	Es zeichnete sich schon früh ab, dass die Arbeit mit dem \textit{LoFive}-Board nicht wie gewollt funktioniert. Der später erfolgte Wechsel auf das \textit{Arduino} hätte schon viel früher passieren sollen. Die vorgegeben Meilensteine bei der Programmierung des \textit{LoFive}-Boards wurden selten eingehalten und und die Frist wurde verlängert. Wegen dem Wechsel des Boards konnte das bisher Programmierte nicht mehr verwendet werden. Dies hat im Endeffekt dazu geführt, dass der komplette Code in kurzer Zeit neu geschrieben werden musste.
	\item Schrittmotoren\\
...
\begin{itemize}
	...
	\item Boardwahl\\
	%TODO Pascal letzter Satz komisch
...
Das Risiko Nr. 10 ist eingetroffen. Nach der vierten Semesterwoche des PREN2 fiel ein Teammitglied aufgrund interner Differenzen aus. Da es sich bei diesem Teammitglied um einen Elektrotechnikstudenten handelte, war dieser Ausfall für unser Team besonders schmerzhaft, da wir nun nur noch einen Elektrotechnik-Student zur Verfügung hatten. Das Eintreffen dieses Risikos bedeutete, dass der verbleibende Elektrotechnikstudent für die ganze Arbeit in seinem Fachbereich zuständig war, was sehr viel zusätzliches Engagement bedeutete.

\subsection{Änderung Mikrokontrollerplattform}
\label{ch:AenderungMikrokontroller}
Das Konzept vom PREN1 beinhaltet als Mikrokontrollerplattform das \textit{LoFive} Board. Wie sich jedoch herausstellte, war es für uns unmöglich diesen Mikrokontroller für unsere Aufgaben zu programmieren. Aus diesem Grund wurde in Semesterwoche drei der Entscheid gefällt, dieses durch einen anderen Mikrokontroller, also durch das \textit{ArduinoDue} zu ersetzen. Das eingetretene Risiko beinhaltet viele nicht brauchbare beziehungsweise nur halbwegs brauchbare Arbeitsstunden des Programmieren für das \textit{LoFive}.

\section{Fazit}
Trotz erschwerten Bedingungen, wie den Austritt von gleich zwei Teammitglieder oder dem Wechsel des ET-Boards im PREN2, konnte das Projekt insgesamt erfolgreich abgeschlossen werden. Ein Grund dafür war das bestreben, eine möglichst einfache Lösung zu finden. Da dies gelang, konnte eine Kombination gefunden werden, die im Vergleich zu anderen Möglichkeiten wenig Aufwand bereiteten und dennoch zuverlässig funktioniert.
...
Das Budget konnte problemlos eingehalten werden. Aufgrund von noch verfügbarem Budget hätte auch im Falle eines Defektes Ersatzteile besorgt werden können.\\
...
\begin{tabular}{|p{.05\textwidth}|p{0.07\textwidth}|p{0.3\textwidth}|p{0.55\textwidth}|}
	...
	\hline
	4.3 & M & Gewicht & 50-90g, siehe Sektion \ref{app:sec:RechKlotzMasse} \\ %todo Berechnung Klotz?
	\hline
