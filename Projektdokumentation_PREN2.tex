...
Zwischen dem RasPi und dem Arduino Due soll der Informationsaustausch über eine serielle Schnittstelle erfolgen.

Für die Kommunikation mittels asynchroner, seriellen UART-Schnittstelle können, auf dem Arduino Due wie auf dem RasPi, die explizit dafür ausgelegten PINs verwendet werden.

Wie in Abb. \ref{fig:RaspberryPins} dargestellt sind dies Pin 18 (Tx) und Pin 19 (Rx). Zusätzlich müssen die beiden Ground (GND) Potentiale vom RasPi und dem Arduino Due verbunden sein.
\begin{figure}[h!]
	\centering
...
\begin{itemize}
		...
		\item Die geplante Durchhangstabelle wird nicht miteinbezogen, da ebenfalls ein Ultraschallsensor verwendet wird. Der Schrittmotor wird aber noch immer einbezogen, da der Ultraschallsensor an der Grundplatte angebracht ist aber die Position der Last angezeigt werden muss.
\end{itemize}
...
\begin{itemize}
	...
	\item Der Kippschalter am ET-Board und der Reset-Knopf auf dem ArduinoDue muss nun betätigt werden um das Gerät zu starten.
\end{itemize}
